...
class Migrator
{
	...
	{
		$this->base = VENDOR_PATH.'Database/';
		$this->migrationFiles = BASE_PATH."database/migrations/";
		$this->schemaFiles = BASE_PATH."database/schema/";
		$this->stubsPath = $this->base."Migrations/stubs/";
		$this->schemaName = "mysql-schema.sql";

		$this->table = "tbl_migrations";
		$this->query = new Builder($this->schemaName, $this->schemaFiles);
		$this->repository = new DatabaseMigrationRepository($this->schemaName, $this->table);

		// prevent unwanted changes against database host specified
		$this->isAllowed = $this->query->isAllowed();
	}
		...
		if($this->isAllowed){
			...
			if(count($pending_migrations) > 0){
				foreach($pending_migrations as $file){
					...
					if($schema != ""){
						$query_response = $this->query->raw($schema);
						if($query_response[val]){
			...
			$response .= nl2br("Running host {$this->query->mysqlHost()} is forbidden.\nPlease check your variables.\n");
		}
	...
	{
		$response = "";
		$isExist = $this->query->tableExist($this->table);
		if($isExist == 0){
	...
	public function migrateFresh()
	{
		$response = "";
		if($this->isAllowed){
			$response .= $this->query->dropAllTables();
			$response .= $this->runStoredDbSchema();
			$response .= $this->runPending();
			$this->query->viewsSchema();
		}else{
			$response .= nl2br("Running host {$this->query->mysqlHost()} is forbidden.\nPlease check your variables.\n");
		}
		return $response;
	...
	public function runStoredDbSchema()
	{
		...
		if(file_exists($file)){
			...
			$res = $this->query->importStoredDatabaseSchema();
	...
	public function dumpBuild($option)
	{
		...
		if($this->isAllowed){
			...
			$this->ensureSchemaBinExist();
			$res = $this->query->schemaDump($option);
			if($res == 0){
				if($option == "prune"){$this->prune();}
